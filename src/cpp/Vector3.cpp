...
{
  Fixed64 radians = angle * Fixed64::DegToRad;
...
Vector3 Vector3::decayKnockbackVector(const Vector3 &vec, Fixed64 deltaTime,
                                      Fixed64 decaySpeed)
{
  Fixed64 resultX = lerpScalar(vec.x(), Fixed64::Zero, decaySpeed);
  Fixed64 resultY = lerpScalar(vec.y(), Fixed64::Zero, decaySpeed);
  Fixed64 resultZ = lerpScalar(vec.z(), Fixed64::Zero, decaySpeed);
...
bool Vector3::velocityToForward()
{
  ...
  if (velocity.lengthSqr() > Fixed64::Epsilon)
  {
    Fixed64 direction = velocity._x >= 0 ? Fixed64::One : Fixed64::MinusOne;
