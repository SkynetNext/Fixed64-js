 ...
 public:
  ...
  constexpr Fixed64 &operator/=(const Fixed64 &rhs) {
    if (rhs.value == 0) {
      ...
      // In division, the precision factor is usually shifted left before the
      // division to maintain precision
  ...
  friend Fixed64 operator%(Fixed64 a, int b) {
    a.value %=
        ...
        << FixLut::PRECISION;  // Modulo operation between Fixed64 and integer
    return a;
  }

  friend Fixed64 operator%(int a, Fixed64 b) {
    b.value = (static_cast<int64_t>(a) << FixLut::PRECISION) %
              b.value;  // Modulo operation between integer and Fixed64
    return b;
  }

  // Comparison operators
  friend bool operator<(Fixed64 a, Fixed64 b) {
    return a.value < b.value;
  }  // Less than

  friend bool operator<(Fixed64 a, int b) {
    return a.value < (static_cast<int64_t>(b)
                      << FixLut::PRECISION);  // Less than with integer
  }

  friend bool operator<(int a, Fixed64 b) {
    return (static_cast<int64_t>(a) << FixLut::PRECISION) <
           b.value;  // Less than with integer
  }
  ...
  friend bool operator<=(Fixed64 a, int b) {
    return a.value <=
           (static_cast<int64_t>(b)
            << FixLut::PRECISION);  // Less than or equal to with integer
  }

  friend bool operator<=(int a, Fixed64 b) {
    return (static_cast<int64_t>(a) << FixLut::PRECISION) <=
           b.value;  // Less than or equal to with integer
  }
  ...
  friend bool operator>(Fixed64 a, int b) {
    return a.value > (static_cast<int64_t>(b)
                      << FixLut::PRECISION);  // Greater than with integer
  }

  friend bool operator>(int a, Fixed64 b) {
    return (static_cast<int64_t>(a) << FixLut::PRECISION) >
           b.value;  // Greater than with integer
  }
  ...
  friend bool operator>=(Fixed64 a, int b) {
    return a.value >=
           (static_cast<int64_t>(b)
            << FixLut::PRECISION);  // Greater than or equal to with integer
  }

  friend bool operator>=(int a, Fixed64 b) {
    return (static_cast<int64_t>(a) << FixLut::PRECISION) >=
           b.value;  // Greater than or equal to with integer
  }
  ...
  friend bool operator==(Fixed64 a, int b) {
    return a.value == (static_cast<int64_t>(b)
                       << FixLut::PRECISION);  // Equal to with integer
  }

  friend bool operator==(int a, Fixed64 b) {
    return (static_cast<int64_t>(a) << FixLut::PRECISION) ==
           b.value;  // Equal to with integer
  }
  ...
  friend bool operator!=(Fixed64 a, int b) {
    return a.value != (static_cast<int64_t>(b)
                       << FixLut::PRECISION);  // Not equal to with integer
  }

  friend bool operator!=(int a, Fixed64 b) {
    return (static_cast<int64_t>(a) << FixLut::PRECISION) !=
           b.value;  // Not equal to with integer
  }

  // Check for infinity
  static bool IsInfinity(Fixed64 value) {
    return value.value == Fixed64::NegativeInfinity ||
           value.value == Fixed64::PositiveInfinity;  // Determines if the value
                                                      // is infinity
  }

  // Check for NaN
  static bool IsNaN(Fixed64 value) {
    return value.value == Fixed64::NaN;
  }  // Determines if the value is NaN

  // Comparison function
  int CompareTo(Fixed64 other) const {
    if (value < other.value) return -1;  // Less than
    if (value > other.value) return 1;   // Greater than
    return 0;                            // Equal
  }

  // Equality function
  bool Equals(Fixed64 other) const {
    return value == other.value;
  }  // Determines if two Fixed64 values are equal

  // Override Object's Equals method
  bool Equals(const Fixed64 &obj) const {
  ...
  }  // Computes the hash code for the value
...
class Fixed64Const {
 public:
  ...
  static constexpr Fixed64 UsableMax =
      Fixed64(static_cast<int64_t>(2147483647LL));
  static constexpr Fixed64 UsableMin =
      Fixed64(static_cast<int64_t>(-2147483648LL));
