...
public:
  Vector3() : _x(Fixed64::Zero), _y(Fixed64::Zero), _z(Fixed64::Zero) {}
  ...
  Vector3(const Vector3Wrapper &other)
      : _x(other.x), _y(other.y), _z(other.z) {}

  operator Vector3Wrapper() const
  {
    return Vector3Wrapper{.x = _x, .y = _y, .z = _z};
  }
  ...
  bool isZero() const
  {
    return _x == Fixed64::Zero && _y == Fixed64::Zero && _z == Fixed64::Zero;
  }
                                     ...
                                     Fixed64 *ptrDistSquared = nullptr)
  {
    ...
    {
      minDistSquared =
          Fixed64::Zero; // 如果内半径小于圆形半径，设置最小距离平方为0
    }
